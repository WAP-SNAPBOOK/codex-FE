...
const validateSlots = (slots, label) => {
  if (slots.some(({ start, end }) => start >= end)) {
    notify(`${label}시작 시간이 종료 시간보다 같거나 늦습니다.`);
    return false;
  }
  if (hasOverlap(slots)) {
    notify(`${label}운영 시간이 겹치는 구간이 있습니다.`);
    return false;
  }
  return true;
};
...
const getSlotsErrors = (slots, label) => {
  ...
  if (hasOverlap(slots)) return [`${label}운영 시간이 겹치는 구간이 있습니다.`];
  return [];
};
...
export const validateStep4 = ({ items }) => {
  ...
  if (hasInvalidMenu) {
    notify('카테고리, 메뉴명, 메뉴 설명, 가격을 모두 입력해주세요.');
    return false;
  }
